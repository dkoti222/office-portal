...
const OfferRelease = ({ navigation }) => {
  ...
  const OfferReleaseItems = items.filter((ele) => ele.status === 'OfferRelease');
  ...
  return (
    <SafeAreaView style={{ flex: 1, alignItems: 'center' }}>
      {OfferReleaseItems.length > 0 ? (  <FlatList data={OfferReleaseItems} renderItem={renderItem} />
  ) : (
