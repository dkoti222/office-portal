...
const OfferHold = ({ navigation }) => {
  ...
  const OfferHoldItems = items.filter((ele) => ele.status === 'OfferHold');
  ...
  return (
    <SafeAreaView style={{ flex: 1, alignItems: 'center' }}>
      {OfferHoldItems.length > 0 ? (  <FlatList data={OfferHoldItems} renderItem={renderItem} />
  ) : (
